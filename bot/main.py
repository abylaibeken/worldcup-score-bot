...
async def show_matches_page(message_or_callback, page: int = 0):
    ...
    async with async_session() as session:
        ...
        total_matches = await session.scalar(
            select(func.count(Match.id)).where(Match.kickoff_at >= datetime.now())
        )
    ...
    if not matches_list:
        ...
        text = f"Ближайшие матчи — страница {page + 1}/{total_pages}:\n\n"

        inline_keyboard = []

        for match in matches_list:
            date_text = match.kickoff_at.strftime("%d.%m.%Y %H:%M")
...
async def predict(message: Message):
    ...
    if len(parts) != 3:
        await message.answer(
            "Отправь прогноз так:\n\n"
            "/predict 1 2:1\n\n"
            "Где 1 — ID матча, 2:1 — твой прогноз."
        )
        return

    try:
        match_id = int(parts[1])
        pred_home, pred_away = map(int, parts[2].split(":"))
    except ValueError:
        await message.answer("Формат: /predict 1 2:1")
        return
    ...
    async with async_session() as session:
        ...
        if existing_prediction:
            await message.answer(
                "Ты уже сделал прогноз на этот матч. Изменить его нельзя."
            )
            return
    ...
    await message.answer(
        f"Прогноз сохранён ✅\n"
        f"{match.home_team} — {match.away_team} {pred_home}:{pred_away}"
    )
...
async def predict_match_callback(callback: CallbackQuery):
    ...
    if datetime.now() >= match.kickoff_at:
        await callback.message.answer("Прогнозы на этот матч уже закрыты.")
        await callback.answer()
        return
    ...
    for home in range(6):
        ...
        for away in range(6):
            row.append(
                InlineKeyboardButton(
                    text=f"{home}:{away}",
                    callback_data=f"score_{match_id}_{home}_{away}",
                )
            )
...
async def import_matches(message: Message):
    ...
    await message.answer(f"Импорт завершён. Добавлено матчей: {added_count}")
...
async def set_result(message: Message):
    ...
    if len(parts) != 3:
        await message.answer("Формат: /result 1 2:1")
        return

    try:
        match_id = int(parts[1])
        real_home, real_away = map(int, parts[2].split(":"))
    except ValueError:
        await message.answer("Формат: /result 1 2:1")
        return
    ...
    async with async_session() as session:
        ...
        for user in users:
            ...
            user.points = sum(p.points for p in user_predictions)

        await session.commit()

    await message.answer(
        f"Результат сохранён:\n"
        f"{match.home_team} — {match.away_team} {real_home}:{real_away}\n"
        f"Очки пересчитаны."
    )
...
async def stats(message: Message):
    ...
    async with async_session() as session:
        ...
        predictions_count = await session.scalar(select(func.count(Prediction.id)))
    ...
    await message.answer(
        ...
        f"Завершённых матчей: {finished_matches_count}"
    )
...
async def table(message: Message):
    ...
    for index, user in enumerate(users, start=1):
        ...
        text += f"{index}. {name} — {user.points} очков\n"
