...
async def show_matches_page(message_or_callback, page: int = 0):
    ...
    if not matches_list:
        text_message = "Матчей на этой странице нет."
...
async def predict(message: Message):
    ...
    async with async_session() as session:
        ...
        if existing_prediction:
            await message.answer("Ты уже сделал прогноз на этот матч.")
            return
...
async def score_callback(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if existing_prediction:
            await callback.message.answer("Ты уже сделал прогноз на этот матч.")
            await callback.answer()
            return
...
async def table(message: Message):
    ...
    for index, user in enumerate(users, start=1):
        ...
        text_message += (
            f"{index}. {name} — {user.points} очков\n"
        )
...
async def reset_all(message: Message):
    ...
    await message.answer(
        ...
        "ID матчей и пользователей сброшены."
    )
